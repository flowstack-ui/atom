...
Virtual anchors accept a plain rectangle or a browser `DOMRect`/`DOMRectReadOnly`
directly from `getBoundingClientRect()`. Return current viewport geometry from
`positioning.getAnchorRect`; returning `null` falls back to the normal reference.

Presentation adapters can call `markPopoverPart(wrapper, "arrow")` when wrapping
Arrow. This preserves server-visible anatomy so Content keeps the Arrow outside
its clipping viewport. Ordinary applications should use the supplied parts.

## Controller, positioning and lifecycle

Use `usePopover` with `Popover.RootProvider` for external control, or Root
for local state. Use Popover.State or usePopoverState for state and methods;
do not mutate legacy context refs. Unique Trigger values identify shared
triggers. The active trigger owns anchoring and focus return.
Root positioning controls collision, fixed positioning, sameWidth, offsets,
virtual anchors and detachment. Content side/align/sideOffset remain shorthands;
explicit positioning fields override their corresponding shorthand.
Root outside events are cancellable. closeOnInteractOutside=false prevents
both pointer and focus dismissal; it does not trap focus. lazyMount,
unmountOnExit and immediate default true to preserve previous mounting behavior.
Retained closed content is hidden. Activity falls back to display-none on React
versions without Activity. Disabled suppresses effective open state without
writing a controlled value. Content supports asChild/render while preserving
the owned viewport and direct Arrow. Use ids for deterministic identifiers and
native ARIA for naming. Application text is not generated by Popover.

`onRequestDismiss` reports an ancestor layer leaving the active layer stack
while this panel remains mounted; preventDefault cancels the child's close
request, not the ancestor's state change. Ordinary close actions use
`onOpenChange`. Removing the active Trigger requests a programmatic close and
clears the active value; controlled owners remain responsible for accepting
these requests. Default-open shared panels should provide defaultTriggerValue.
