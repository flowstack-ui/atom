...
test("FileUpload source wires controlled state, clear refocus, and keyboard trigger behavior", async () => {
  ...
  assert.match(rootSource, /setResolvedFiles\(resolvedFiles\.filter\(\(currentFile\) => currentFile !== file\)\)/);
  ...
  assert.match(triggerSource, /if \(event\.defaultPrevented\) return;\s*openFilePicker\(\);/);
  ...
  assert.match(triggerSource, /"aria-describedby": restProps\["aria-describedby"\] \?\? host.describedBy \?\? ctx\.describedBy/);
  ...
  assert.match(deleteTriggerSource, /if \(event\.defaultPrevented\) return;\s*removeFile\(file\);/);
  ...
  assert.match(dropzoneSource, /if \(isInactive \|\| !ctx.allowDrop\) return;\s*if \(event\.currentTarget\.contains/);
  assert.match(dropzoneSource, /\[isInactive, setDragState, ctx.allowDrop\]/);
});
