...
try {
  ...
  await writeFile(
    ...
    [
      ...
      'import { Menu, useMenu, type MenuPositioningOptions, type MenuHighlightTarget } from "@flowstack-ui/atom/menu";',
      'import { DropdownMenu, useDropdownMenu } from "@flowstack-ui/atom/dropdown-menu";',
      'import { ContextMenu, useContextMenu } from "@flowstack-ui/atom/context-menu";',
      'import { Menubar, useMenubar, type MenuSelectionEvent as MenubarSelectionEvent } from "@flowstack-ui/atom/menubar";',
      'import { NavigationMenu, useNavigationMenu, useNavigationMenuContext, type NavigationMenuApi, type NavigationMenuContentProps } from "@flowstack-ui/atom/navigation-menu";',
      'declare const publicNavigationContext: ReturnType<typeof useNavigationMenuContext>;',
      'const publicNavigationApi: NavigationMenuApi = publicNavigationContext;',
      '// @ts-expect-error Internal registries are not public context API.',
      'publicNavigationContext.registerItem;',
      'void publicNavigationApi;',
      'const menuPosition: MenuPositioningOptions = { placement: "bottom-start", sameWidth: true, fitViewport: true, arrowPadding: 8 };',
      'const menuHighlight: MenuHighlightTarget = { value: "default", groupId: "density" };',
      'const menuCancel = (event: MenubarSelectionEvent) => event.preventDefault();',
      'const navigationContent: Pick<NavigationMenuContentProps, "asChild"> = { asChild: true };',
      'void [Menu.RootProvider, Menu.Context, useMenu, DropdownMenu.RootProvider, DropdownMenu.Context, useDropdownMenu, ContextMenu.RootProvider, ContextMenu.Context, useContextMenu, Menubar.RootProvider, Menubar.Context, useMenubar, NavigationMenu.RootProvider, NavigationMenu.Context, NavigationMenu.ItemIndicator, useNavigationMenu, menuPosition, menuHighlight, menuCancel, navigationContent];',
      ...
      'import { useSelection, useSelectionCheckbox, type SelectionOptions } from "@flowstack-ui/atom/selection";',
      'import { ActionDelegate, type ActionDelegateProps } from "@flowstack-ui/atom/action-delegate";',
      'import type { CheckboxRootProps } from "@flowstack-ui/atom/checkbox";',
      'const selectionOptions: SelectionOptions = { orderedKeys: ["a", "b"] as const, defaultSelectedKeys: ["a"] as const };',
      'const checkboxBinding: Pick<CheckboxRootProps, "checked" | "disabled" | "readOnly" | "onCheckedChange"> = {} as ReturnType<typeof useSelectionCheckbox>;',
      '// @ts-expect-error Selection IDs are strings, not numeric row indices.',
      'const invalidSelection: SelectionOptions = { orderedKeys: [42] };',
      '// @ts-expect-error A delegate requires a real target ID.',
      'const invalidDelegate: ActionDelegateProps = { children: null };',
      'void [useSelection, useSelectionCheckbox, ActionDelegate, selectionOptions, checkboxBinding, invalidSelection, invalidDelegate];',
      'import { SelectRoot, SelectTrigger } from "@flowstack-ui/atom/select";',
      'import { Collapsible, useCollapsible, type UseCollapsibleOptions } from "@flowstack-ui/atom/collapsible";',
      'const disclosureOptions: UseCollapsibleOptions = { lazyMount: true, unmountOnExit: false, collapsedHeight: "5rem", hideMode: "display-none", ids: { content: "details" } };',
      'void [Collapsible.RootProvider, Collapsible.Context, Collapsible.Indicator, useCollapsible, disclosureOptions];',
  ...
  await writeFile(
    ...
    [
      ...
      'const originalConsoleError = console.error; const serverWarnings = []; console.error = (...args) => { serverWarnings.push(args.map(String).join(" ")); originalConsoleError(...args); };',
      'const retainedSubmenu = React.createElement(Menu.Sub, { lazyMount: false, unmountOnExit: false }, React.createElement(Menu.SubTrigger, { value: "more" }, "More"), React.createElement(Menu.Portal, { disabled: true }, React.createElement(Menu.SubContent, null, React.createElement(Menu.Item, { value: "nested" }, "Nested command"))));',
      'const retainedMenu = renderToStaticMarkup(React.createElement(Menu.Root, { lazyMount: false, unmountOnExit: false }, React.createElement(Menu.Portal, { disabled: true }, React.createElement(Menu.Content, { ariaLabel: "Retained commands" }, retainedSubmenu))));',
      'const retainedSurfaces = retainedMenu.match(/<[^>]*role="menu"[^>]*>/g) ?? [];',
      'if (retainedSurfaces.length !== 2 || retainedSurfaces.some(tag => !/\\binert=""/.test(tag))) throw new Error("Closed retained root/submenu must serialize inert in every supported React line: " + JSON.stringify(retainedSurfaces));',
      'console.error = originalConsoleError; if (serverWarnings.length) throw new Error("Retained menu SSR emitted warnings: " + serverWarnings.join("\\n"));',
      'const disclosure = (options) => React.createElement(Collapsible.Root, options, React.createElement(Collapsible.Trigger, null, "Toggle"), React.createElement(Collapsible.Content, null, "Preview"));',
      'const disclosureHtml = renderToStaticMarkup(disclosure({ collapsedHeight: 80 }));',
      'if (!disclosureHtml.includes("inert") || !disclosureHtml.includes("Preview")) throw new Error("Collapsible partial SSR mismatch");',
      'if (!React.Activity) { const hidden = renderToStaticMarkup(disclosure({ hideMode: "activity", lazyMount: false })); const visible = renderToStaticMarkup(disclosure({ hideMode: "activity", open: true })); if (!hidden.includes("inert") || !hidden.includes("hidden") || !visible.includes("Preview")) throw new Error("Collapsible Activity fallback must retain unavailable content inert and render open content"); }',
  ...
  await writeFile(
    ...
    [
      ...
      '    const rootNamespace = rootModule[name];',
      '    if (!rootNamespace) throw new Error(`Missing root namespace: ${name}`);',
      '    for (const part of new Set([...Object.keys(value), ...Object.keys(rootNamespace)])) if (rootNamespace[part] !== value[part]) throw new Error(`Packed namespace mismatch: ${name}.${part}`);',
      '  }',
      '}',
      '',
    ].join("\n"),
