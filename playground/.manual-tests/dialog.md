...
Android device is available. The focused coverage-workbook update is complete.
Do not infer that unexecuted legacy steps below passed; the completed workbook
rows map only to the approved focused qualification described here.
...
| Coverage workbook | Pass: focused Dialog/Modal rows and iPhone evidence updated |
