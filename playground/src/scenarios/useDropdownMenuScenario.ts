...
export type DropdownMenuItemCompositionMode = "default" | "asChild" | "render";
export type DropdownMenuAnchorPoint = {
  x: number;
  y: number;
  localX: number;
  localY: number;
...
export type DropdownMenuRefPart =
  | "content"
  | "group"
  | "item"
  | "disabledItem"
...
export type DropdownMenuRefSnapshot = Record<DropdownMenuRefPart, string>;

export type DropdownMenuPartsSnapshot = {
  rootMode: string;
  defaultOpen: string;
  open: string;
  modal: string;
...
export type DropdownMenuScenarioState = {
  controlled: boolean;
  defaultOpen: boolean;
  open: boolean;
  modal: boolean;
  ...
  itemComposition: DropdownMenuItemCompositionMode;
  ...
  logPointer: boolean;
  log: DropdownMenuLogEntry[];
  refs: DropdownMenuRefSnapshot;
  parts: DropdownMenuPartsSnapshot;
};

export type DropdownMenuScenarioActions = {
  ...
  setItemComposition: (value: DropdownMenuItemCompositionMode) => void;
  ...
  handleTriggerClick: (event: { preventDefault: () => void }) => void;
  handleTriggerKeyDown: (event: { key: string; preventDefault: () => void }) => void;
  ...
  clearLog: () => void;
  markPartRef: (part: DropdownMenuRefPart, element: HTMLElement | null) => void;
};

export function useDropdownMenuScenario() {
  ...
  const [itemComposition, setItemComposition] = useState<DropdownMenuItemCompositionMode>("default");
  ...
  const [log, setLog] = useState<DropdownMenuLogEntry[]>([]);
  const [refs, setRefs] = useState<DropdownMenuRefSnapshot>(emptyDropdownMenuRefs);
  ...
  const markPartRef = useCallback((part: DropdownMenuRefPart, element: HTMLElement | null) => {
    if (!element) return;
    setRefs((current) => {
  ...
  const handleTriggerClick = useCallback((event: { preventDefault: () => void }) => {
  ...
  const handleTriggerKeyDown = useCallback((event: { key: string; preventDefault: () => void }) => {
    if (event.key !== "Enter" && event.key !== " " && event.key !== "ArrowDown" && event.key !== "ArrowUp") return;
    addLog(`trigger user onKeyDown ${event.key === " " ? "Space" : event.key}`);
    if (blockTriggerEvent) {
      event.preventDefault();
      addLog("trigger blocked");
    }
  ...
  const parts = getDropdownMenuParts({
    controlled,
    defaultOpen,
    open,
    modal,
  ...
  const state: DropdownMenuScenarioState = {
    controlled,
    defaultOpen,
    open,
    modal,
  ...
  const actions: DropdownMenuScenarioActions = {
    setControlled,
    setDefaultOpen,
    setControlledOpen,
    setModal,
...
function getDropdownMenuParts({
  ...
  sideOffset: number;
  contentLoopOff: boolean;
}): DropdownMenuPartsSnapshot {
  const trigger = document.querySelector<HTMLElement>("[data-playground-dropdown-menu-trigger]");
...
const emptyDropdownMenuRefs: DropdownMenuRefSnapshot = {
  content: "none",
  group: "none",
  item: "none",
  disabledItem: "none",
